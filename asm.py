...
def asm_pass(iteration, s_file):
    ...
    for source_line in source:
        ...
        # if there is a comment, get rid of it. Comments are after a ';'
	...
	# Personal comment : Next line seems to be equivalent to 'tokens = source_line.split(" ")'
        tokens = re.findall('[\S]+', source_line) # \S means: any non-whitespace
        # print tokens # to debug

        # if there is a label, consume it
	# Personal comment : Label are of type : 'label: code...'
        if tokens:
            token=tokens[0]
        ...
        if tokens:
         ...
         #begin sabote
            ...
            if instruction_encoding=="":
                ...
                # Personal comment : Mainly used to compute the size of the final code
